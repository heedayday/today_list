		  ...
		  <table class="table table-striped" style="text-align: center;  border-radius: 5px; box-shadow: rgba(0,0,0,0.8) 0 0 10px;">
				<thead>					
					<tr> 		
			 		...
			 		<h3><span style="color: white">LAST MONTH</span>&nbsp;&nbsp;&nbsp;<button type="button" onclick="location.href='Register_main_Month.jsp?userID=<%=userID %>'" class="btn btn-secondary">></button></h3></th>
			 		</tr>				
					<tr>
				...
				<%
					...
					String before = beforeYear+"-"+beforeMonth+"-";
				    String userDate=before;
					UserDataDAO userdataDAO = new UserDataDAO();
					ArrayList<UserData> list = userdataDAO.getList2(userID,userDate);
					for(int i=0;i< list.size(); i++){
				%>	
				<tr>				
				<td style="background-color: #fafafa;  text-align:center;"><a href="view_MonthB.jsp?bbsID=<%= list.get(i).getBbsID()%>"><%= list.get(i).getUserTitle()%></a></td> 
				...
				<a href="write_MonthB.jsp?userID=<%=userID%>" style="box-shadow: rgba(0,0,0,0.8) 0 0 5px;" class="btn btn-primary pull-right">글쓰기</a>
