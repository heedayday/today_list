 ...
 //등록하는 페이지	
		...
		var search2Request = new XMLHttpRequest();
 		...
 		function registerFunction(){
 			vCheck(document.getElementById("registerTitle"),Title);
 			vCheck(document.getElementById("registerDate"),Date);
 			vCheck(document.getElementById("registerMemo"),Memo);	
 			...
 			location.href ='Register_main.jsp?userID='+searchParam("userID");
...
<body>
	...
	<!--  <div class="container">-->
		<form method="post" action="writeAction_AT.jsp?userID=<%=userID %>">
			...
			<input type="submit"  style="box-shadow: rgba(0,0,0,0.8) 0 0 3px;" class="btn btn-primary pull-right" value="추가">
		</form>
		<!--  </div>-->
		  ...
		  <table class="table table-striped" style="text-align: center; border-radius: 5px; box-shadow: rgba(0,0,0,0.8) 0 0 10px;">
				<thead>					
					<tr> 		
			 		...
			 		<h3><button type="button" onclick="location.href='Register_main_M2.jsp?userID=<%=userID %>'" class="btn btn-secondary"><</button>&nbsp;&nbsp;&nbsp; <span style="color: white">TOMORROW LIST</span></h3></th>
			 		</tr>				
					<tr>					
				...
				<%			
					...
					String tomorrow = sdf.format(c1.getTime()); // String으로 저장
					String userDate= tomorrow;  
					UserDataDAO userdataDAO = new UserDataDAO();
					ArrayList<UserData> list = userdataDAO.getList3(userID,userDate);
					for(int i=0;i< list.size(); i++){
				%>	
				<tr>				
				<td style="background-color: #fafafa; text-align: center; border-bottom-left-radius: 5px; border-top-left-radius: 5px;"><a href="view_AT.jsp?bbsID=<%= list.get(i).getBbsID()%>"><%= list.get(i).getUserTitle()%></a></td> 				
