...
<body>
	<%
		...
		if(bbsID == 0){
			...
			script.println("location.href ='Register_main_BT.jsp"+"?userID="+name);
			script.println("</script>");
		}
	...
	<div class="container">
		...
		<table class="table" style="text-align: center; border-radius: 5px; box-shadow: rgba(0,0,0,0.8) 0 0 10px;">
			<thead>
				<tr>
					<th colspan="2" style="background-color: #42579D; text-align:center; color: white"><h4>일정 확인하기</h4></th>
				</tr>			
			</thead>		
			<tbody>
				<tr>
		...
		<a href="Register_main_BT.jsp?userID=<%=userID %>" class="btn btn-primary" style="box-shadow: rgba(0,0,0,0.8) 0 0 3px;">목록</a>
		<%
			if(userID != null && userID.equals(bbs.getUserID())){
		%>
			<a href="update_BT.jsp?bbsID=<%= bbsID %>" class="btn btn-primary" style="box-shadow: rgba(0,0,0,0.8) 0 0 3px;">수정</a>
			<a onclick="return confirm('정말로 삭제하시겠습니까?')" href="deleteAction_BT.jsp?bbsID=<%= bbsID %>" class="btn btn-primary" style="box-shadow: rgba(0,0,0,0.8) 0 0 3px;">삭제</a>
